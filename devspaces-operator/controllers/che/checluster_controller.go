...
import (
	...
	"github.com/eclipse-che/che-operator/controllers/wazi"
	...
	deploywazi "github.com/eclipse-che/che-operator/pkg/deploy/wazi"
...
func (r *CheClusterReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	var toWaziLicenseRequestMapper handler.MapFunc = func(obj client.Object) []ctrl.Request {
		isWaziLicenseObj, reconcileRequest := deploywazi.IsWaziLicense(r.nonCachedClient, r.namespace, obj)
		if isWaziLicenseObj {
			return []ctrl.Request{reconcileRequest}
		}
		return []ctrl.Request{}
	...
	controllerBuilder := ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&source.Kind{Type: &chev2.WaziLicense{}},
			handler.EnqueueRequestsFromMapFunc(toWaziLicenseRequestMapper),
			builder.WithPredicates(onAllExceptGenericEventsPredicate),
		)
...
func (r *CheClusterReconciler) GetCR(request ctrl.Request) (*chev2.CheCluster, error) {
	...
	// Determine if the request is a WaziLicense
	if err != nil {
		if errors.IsNotFound(err) {
			wazilicense := &chev2.WaziLicense{}
			err = r.client.Get(context.TODO(), request.NamespacedName, wazilicense)
			if err == nil {
				checluster, _, err = deploy.FindCheClusterCRInNamespace(r.client, request.NamespacedName.Namespace)
			}
		}
	}
