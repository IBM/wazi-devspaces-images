...
```bash
docker build -f devspaces-sidecar-udibase-rhel9/wazi.Dockerfile -t idzee-devspaces-sidecar:6.5.0 ./devspaces-sidecar-udibase-rhel9
```
...
docker buildx build --platform linux/amd64,linux/s390x --tag idzee-devspaces:6.5.0 --builder devspaces-builder -f wazi.Dockerfile .
```
...
Instead of `latest` you can use the version number such as `6.5.0` or the SHA listed in the table below. Here is a list of released images.

| <sub>Registry and Image</sub> | <sub>Version</sub> |
| :--- | --- |
|<sub>icr.io/wazi-code/ibm-wazi-for-devspaces-sidecar@sha256:TBD</sub>|<sub><b>6.5.0</b></sub>|
