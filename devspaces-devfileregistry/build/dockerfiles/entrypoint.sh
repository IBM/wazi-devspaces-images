...
LICENSE_TYPE=${WAZI_LICENSE_USAGE}
...
DEFAULT_RESOURCES_DIR="${DEFAULT_DEVFILES_DIR%/*}"/resources/v2
...
function wazi_license_type() {
  if [[ "${LICENSE_TYPE,,}" == "idzee" ]]; then
    wazi_desc=$(jq -r '(.[] | select(.description | contains("Analyze, "))).description' "$INDEX_JSON")
    wazi_desc="${wazi_desc//Analyze, /}"
    cat <<< $(jq --arg desc "$wazi_desc" '(.[] | select(.description | contains("Analyze, "))).description |= $desc' "$INDEX_JSON") > "$INDEX_JSON"
    mv -fv "${DEFAULT_RESOURCES_DIR}/zopeneditor-sample-idzee.zip" "${DEFAULT_RESOURCES_DIR}/zopeneditor-sample.zip"
  fi
}
